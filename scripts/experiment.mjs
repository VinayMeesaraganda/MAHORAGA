...
if (action === "provision") {
  ...
  const all = await Promise.all(["guidance-continuation", "price-volume"].map(async s => {
    const credential = JSON.parse(await readFile(resolve(root, ".paper-accounts", `${s}.json`), "utf8"));
    return { strategy: s, credential, account: await paperAccount(credential) };
  }));
  ...
  if (new Set([...all.map(a => a.account.id), baselineAccount.id]).size !== 3) throw Error("All three accounts must be distinct");
