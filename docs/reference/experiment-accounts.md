...
| Guidance continuation | `.paper-accounts/guidance-continuation.json` | Dedicated `mahoraga-guidance-continuation` Worker deployed; enabled in paper mode; reviewed issuer events still required, broker fill validation pending |
| Price/volume | `.paper-accounts/price-volume.json` | Dedicated `mahoraga-price-volume` Worker deployed; enabled in paper mode; deterministic entry rules enforced, broker fill validation pending |
