...
## September 14: baseline research timeout repair

The deployed baseline rotated candidates correctly but repeatedly timed out at the 30-second LLM deadline. At 09:29 ET it had nine reserved LLM attempts and only one recorded completion (SMCI, WAIT); live error messages explicitly named the provider timeout. This supersedes the earlier claim that the single research card was merely a healthy queue. The dashboard's percentages are source sentiment/ranking scores, not calibrated probabilities of profit.

The baseline keeps NVIDIA `openai/gpt-oss-20b` and its existing output caps (2048 research, 4096 analyst). Its Worker variable and local `.dev.vars` now set `LLM_EXTRA_BODY={"reasoning_effort":"low"}`. This is provider configuration, not an `/agent/config` field; changing an agent JSON file alone cannot apply it. Keep that non-secret variable in the baseline's private `wrangler.jsonc` when redeploying. Do not apply it blindly to other model providers. NVIDIA documents low/medium/high effort for [gpt-oss-20b](https://docs.api.nvidia.com/nim/reference/openai-gpt-oss-20b).

The signal prompt retains supplied market, macro and catalyst evidence and explicit unknowns, but removes repeated instructions and asks for a two-sentence rationale. Missing headlines now mean unavailable coverage, rather than asserting no news occurred. JSON validation, request reservation/counting, the 30-second deadline, candidate rotation, execution guards and exit ordering remain intact. The dashboard shows recent failed research attempts until a later successful analysis supersedes them (or the log window expires).

Read-only diagnostic: `node scripts/research-probe.mjs --remote --repeat 2`; add `--analyst` to check the batch-analyst prompt. It uses actual prompt builders and response schemas, an observed market packet when available, and unknowns for state the status API does not expose. It sends diagnostic LLM requests but never submits orders or records the output as trading research. These diagnostic calls are outside the Durable Object's daily counter. The original `npm run doctor` connection check uses a tiny prompt and cannot establish real-workload latency.

Before deployment, the original prompt/default effort took 27.8 seconds in a successful probe, alongside the repeated deployed timeouts. Original prompt/low effort took 10.6 seconds; concise prompt/low effort completed twice in 2.8 and 4.0 seconds with valid WAIT results. A batch-analyst probe passed in 26.9 seconds, leaving less latency margin. These measurements are operational samples, not a latency guarantee or strategy-performance evidence. A candidate Llama endpoint returned HTTP 410 and Nemotron probes timed out; neither was selected.

Deployed baseline version `8ead5302-6a4e-4a23-8199-b7913dc69867`, operating revision `research-latency-2026-09-14`. Actual Cloudflare alarms then completed ELMT (09:38:05–09:38:08 ET) and NCT.X (09:39:05–09:39:07 ET), both WAIT; the status API held four research records. Existing records, request counters and paper account history were retained. The remote baseline stayed enabled and the local baseline stayed disabled. Typecheck, all 671 tests in 51 files, and the dashboard production build passed. No diagnostic orders were submitted; autonomous trading remains subject to the existing entry/risk gates.

Separate dependency gaps remain visible: the baseline news gatherer reported its pagination budget exceeded, and Reddit/insider feeds were unavailable. This repair does not certify those sources or fabricate catalysts to force entries. The guidance and price/volume Workers use deterministic strategy evaluation and do not depend on this LLM configuration.
