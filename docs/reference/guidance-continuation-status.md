...
September 13 follow-up: the [three-strategy operating guide](multi-strategy-operation.md) supersedes the runtime-wiring and deployment gaps below. Guidance now has a dedicated Worker and account-pinned execution path, enabled in paper mode by explicit operator authorization. Prospective Finnhub collection is working; comparable reviewed issuer evidence remains required for entries. Actual broker fill/protection validation is pending and was not asserted as part of authorization. Earlier shadow-only research APIs retain their original behavior.
...
Cloud deployment update, September 13: a separate Cloudflare D1 database and KV namespace were created, all five migrations applied remotely, and the `mahoraga-paper` Worker was deployed in paper-only mode. Its authenticated broker/configuration readback showed an ACTIVE $100,000 paper account, zero positions and pending orders, equities-only policy, and options/crypto disabled; the baseline profile was then applied and the remote harness enabled. The guidance-continuation path remains shadow-only and cannot submit broker orders. Remote deployment is not evidence of a strategy edge, and the source/evidence and broker-acceptance prerequisites above still apply.
